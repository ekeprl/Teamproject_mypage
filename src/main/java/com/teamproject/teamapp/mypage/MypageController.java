...
public class MypageController {
	...
	public String edit(MypageVO vo,@RequestParam String memnPass, HttpSession session ) {
        ...
        = Hashing.sha256().hashString(vo.getMemPass(), StandardCharsets.UTF_8)
        				  .toString();
		String encodedPass
        = Hashing.sha256().hashString(memnPass, StandardCharsets.UTF_8)
        				  .toString();
		MypageVO storeduser = mypageService.selectMypage(vo.getMemId());
		if(resultHexString.equals(storeduser.getMemPass()) ) {
			storeduser.setMemPass(encodedPass);
			int num = mypageService.updateMypage(storeduser);
			session.invalidate();
			return "redirect:/mypage/login.do";
		}
		  else { return "redirect:/mypage/list.do"; }
	}
		//개인 정보 수정 jsp
	...
	public String del(MypageVO vo){
        ...
        = Hashing.sha256().hashString(curpass, StandardCharsets.UTF_8)
        				  .toString();
...
//			session.removeAttribute("loginUser");		//속성을 삭제
		...
		@RequestMapping(value = "/mypage/hostadd.do", method = RequestMethod.GET)
		public String hostaddForm(String memId,HttpSession session,Map<String,Object>map) {
			MypageVO vo = mypageService.selectMypage(memId);
			map.put("memVo", vo);
			return "mypage/hostAdd";
		}
}
